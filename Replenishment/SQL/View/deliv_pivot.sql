 /* delivery invoice view */
create view deliv_pivot_invoice as
SELECT delivery.store,
    item_support.mupc,
    sum(delivery.qty) AS deliveries,
    item_support.item_desc,
	item_support.display_size,
    item_support.season,
    item_support.total_case_size
FROM delivery
JOIN item_support ON delivery.upc = item_support.upc
WHERE delivery.type = 'Invoice' or delivery.type = 'BandAid'
GROUP BY delivery.store, item_support.mupc, item_support.item_desc, item_support.season, item_support.total_case_size, item_support.display_size
ORDER BY delivery.store;


 /* delivery credit view */
create view deliv_pivot_credit as
SELECT delivery.store,
    item_support.mupc,
...
GROUP BY delivery.store, item_support.mupc, item_support.item_desc, item_support.season, item_support.total_case_size
ORDER BY delivery.store;
